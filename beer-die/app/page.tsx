...
function getHomeData() {
  ...
  const activeGames = db.prepare(`
    ...
    WHERE g.status='active' ORDER BY g.started_at DESC
  `).all() as any[];
  const recentGames = db.prepare(`
    SELECT g.*, p1.name AS t1p1_name, p2.name AS t1p2_name, p3.name AS t2p1_name, p4.name AS t2p2_name
    FROM games g JOIN players p1 ON p1.id=g.team1_p1 JOIN players p2 ON p2.id=g.team1_p2
    JOIN players p3 ON p3.id=g.team2_p1 JOIN players p4 ON p4.id=g.team2_p2
    WHERE g.status='finished' ORDER BY g.ended_at DESC LIMIT 5
  `).all() as any[];
  const topPlayers = db.prepare(`
    ...
    GROUP BY p.id HAVING games>0 ORDER BY wins DESC, sinks DESC LIMIT 5
  `).all() as any[];
  const totals = db.prepare(`
    SELECT COUNT(*) AS total_games, COUNT(CASE WHEN status='active' THEN 1 END) AS active_games,
      (SELECT COUNT(*) FROM events WHERE throw_result='sink') AS total_sinks,
      (SELECT COUNT(*) FROM players) AS total_players FROM games
  `).get() as any;
  return { activeGames, recentGames, topPlayers, totals };
}

export default function HomePage() {
  const { activeGames, recentGames, topPlayers, totals } = getHomeData();
  return (
    <div className="max-w-5xl mx-auto px-4 py-8 space-y-8">
      ...
      <div className="grid grid-cols-4 gap-3">
        {[{label:'Games Played',value:totals?.total_games??0},{label:'Active Games',value:totals?.active_games??0,highlight:true},{label:'Total Sinks',value:totals?.total_sinks??0},{label:'Players',value:totals?.total_players??0}].map(s=>(
      ...
      {activeGames.length>0&&(
        <section>
          <div className="flex items-center gap-2 mb-3">
            <Zap className="w-5 h-5 text-amber-400" /><h2 className="text-xl font-bold">Live Games</h2>
              ...
              <Link key={g.id} href={`/game/${g.id}`} className="block rounded-xl border border-amber-500/30 bg-amber-500/5 hover:bg-amber-500/10 p-4 transition-colors">
                <div className="flex items-center justify-between">
                  <div className="flex-1">
                    <div className="flex items-center gap-3"><span className="font-semibold text-white">{g.team1_name}</span><span className="text-slate-400 text-xs">{g.t1p1_name} & {g.t1p2_name}</span></div>
                    <div className="flex items-center gap-3 mt-1"><span className="font-semibold text-white">{g.team2_name}</span><span className="text-slate-400 text-xs">{g.t2p1_name} & {g.t2p2_name}</span></div>
                  </div>
      ...
      <div className="grid md:grid-cols-2 gap-6">
        <section>
          ...
          {recentGames.length===0?(
            <div className="rounded-xl border border-slate-800 bg-slate-900 p-6 text-center text-slate-500">No games yet. <Link href="/game/new" className="text-amber-400 hover:underline">Start one!</Link></div>
          ):(
        ...
        <section>
          ...
          {topPlayers.length===0?(
            <div className="rounded-xl border border-slate-800 bg-slate-900 p-6 text-center text-slate-500">No players yet. <Link href="/players" className="text-amber-400 hover:underline">Add players!</Link></div>
          ):(
            <div className="space-y-2">
              {topPlayers.map((p:any,i:number)=>(
                <Link key={p.id} href={`/players/${p.id}`} className="flex items-center gap-3 rounded-xl border border-slate-800 bg-slate-900 hover:bg-slate-800/80 p-3 transition-colors">
                  <span className={`w-7 h-7 rounded-full flex items-center justify-center text-sm font-black ${i===0?'bg-amber-500 text-black':i===1?'bg-slate-400 text-black':i===2?'bg-amber-700 text-white':'bg-slate-700 text-slate-300'}`}>{i+1}</span>
                  <div className="flex-1"><div className="font-semibold text-white">{p.nickname||p.name}</div><div className="text-xs text-slate-400">{p.wins}W · {p.games-p.wins}L · {p.sinks} sinks</div></div>
                  <div className="text-right"><div className="text-amber-400 font-bold">{p.games>0?Math.round((p.wins/p.games)*100):0}%</div><div className="text-xs text-slate-500">win rate</div></div>
                </Link>
              ))}
            </div>
          )}
